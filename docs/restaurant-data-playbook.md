...
|---|---|---|---|
| Chick-fil-A | `full` | Direct WebFetch of chick-fil-a.com/menu item pages for single-size items. **For multi-size items (fries, mac & cheese), the item pages only render one size via WebFetch's markdown conversion** — instead `curl` `chick-fil-a.com/nutrition-allergens` directly and extract the page's own embedded WordPress Interactivity API JSON state (`nutrition-allergens-table-store.activeTableData`), which lists every published size as `sub_items` per item. Still first-party, just a different path on the same domain. | — |
...
| Dairy Queen | `full` (existing 9 items); size-completion pass **blocked** | Initial batch: national nutrition brochure PDF (dotcmscloud.com CDN) for numbers; WebSearch snippet extraction for per-item ingredient pages | Direct WebFetch to dairyqueen.com is blocked by bot protection, confirmed twice. A size-completion retry found a *different*, dated brochure on the same CDN pattern (`Q3-2022-US-TREAT-BROCHURE.pdf`) whose Medium-size Blizzard numbers **don't match what's already shipped** (e.g. OREO Blizzard: repo has 790 cal, that PDF has 820) — almost certainly a stale artifact DQ left on their CDN after republishing (the original ingestion agent's retrieval, dated 2026-07, is presumably the current one), but this hasn't been independently confirmed and is worth a spot-check alongside the rest of the batch. Current-generation numbers for the missing sizes (Mini/Small/Large) exist only via aggregators, correctly refused per the trust ranking below. **Needs a connected browser session or a supplied document**, same as Panda Express, to add the missing sizes or to positively confirm the existing Medium data is current. |
...
| Panda Express | *(blocked — attempted twice)* | — | pandaexpress.com is fully DataDome-gated: 403 on every plain-UA variant tried (`curl/8.4.0`, `curl/7.68.0`, Googlebot, generic browser UA), and unlike Panera's Akamai wall, the UA trick doesn't transfer. WebSearch snippets (the method that unblocked Dairy Queen) don't transfer either here — search results only surface aggregator re-hosts and one dead-end S3 artifact (`s3.amazonaws.com/PandaExpressWebsite/files/pdf/Nutrition.pdf` → 404/`NoSuchKey`, stale index entry, don't re-chase it). Two attempts have now exhausted every automated path; this chain **needs either a connected claude-in-chrome browser session or Mark supplying a source document directly** (a PDF he downloads himself, or a photo of an in-store nutrition brochure). Aggregators found here visibly disagree with each other on the same item (Orange Chicken: 490 vs 510 cal across two sources) — a live illustration of why the trust ranking below refuses them outright. |
