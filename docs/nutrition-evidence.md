...
| **Budget nutrients** (sat fat, sodium): in a nutrient-dense food (protein *or* fiber ≥20% DV) where one is the *lone* elevated nutrient, it does not warn — it reframes to "everyday, just budget it across the day," with the trade-off named in the headline. Sat fat: <20% "good", 20–24% "moderate", ≥25% still warns. Sodium: 20–39% "moderate" (budgeted), ≥40% still warns. If both are high it's two concerns and neither budgets (warns) | Both are *daily-total* concerns (DGA/AHA ~20 g sat fat/day; ~2300 mg sodium/day), not per-serving quality defects — the FDA 20% "high" line is per serving for both. A use-pattern reframe, **not** a claim that protein/fiber physiologically offsets them, so the trade-off stays visible rather than silently passed. Ceilings keep ~2 servings within a day's budget | A (consensus guidance) |
| **Sodium budget does NOT apply when the `bp` condition is set** — sodium stays a flag, not a budget caveat, for someone managing blood pressure | The whole point of the bp personalization is to take sodium *more* seriously; the general-population "budget it" reframe would undercut that | A (consensus guidance) |
...
| Scanned items in a matrix-destroyed category (spec 008 M2 — `en:fruit-juices`, `en:sodas`, `en:smoothies`, sweet/carbonated beverages, from OFF `categories_tags`): sugar scored on TOTAL even when the added-sugar label reads 0, because WHO counts these as free sugar (juicing/processing destroys the fruit matrix). Category tags used only as a veto (force caution), never to grant an exemption; the human whole-food flag outranks it. The `blood_sugar` note also fires on total here (glycemic load is the total sugar) | WHO free-sugars guideline classifies fruit juice as free sugar; whole-fruit-vs-juice T2D cohort divergence (spec 007 sources) | A (regulatory consensus + human trial) |
| Restaurant items marked `wholeFoodSugarMatrix` (spec 007): total sugar is fully exempted from the sugar tone/threshold — never drives warn or moderate, never enters `highNutrients`. Context-only line surfaces the raw %DV instead. `blood_sugar` condition's profileNote is unaffected (fires on the raw number — glycemic load is real regardless of matrix). Gated on an intact whole-food solid matrix (solid, cut-not-blended, fiber-bearing), **not** on natural-vs-added origin — so juices/smoothies/syrup-packed items are correctly excluded. Human-reviewed per item, requires `wholeFoodSugarBasis`; never inferred from ingredient text | The health signal is the food matrix and physical form, not origin: the sugar molecule is identical regardless of source (Eur J Nutr 2024 review), and WHO classifies natural fruit *juice* as free sugar precisely because processing destroys the matrix. Whole-fruit intake tracks with neutral/lower T2D risk, juice with higher (Harvard cohorts). Confounding caveat: some whole-fruit benefit is diet-quality confounding (EPIC-NL), but the mechanistic matrix evidence (glycemic response, satiety, apple-vs-juice LDL) is interventional | A (regulatory consensus + human trial) |
...
- **Verdict-moving vs. context-only:** only the tagged verdict rules (offsets, the
  sat-fat/sodium budget reframes, trans flag, condition/goal thresholds) can shift good/ok/warn.
