...
| Restaurant items marked `intrinsicSugarOnly` (spec 007): total sugar is fully exempted from the sugar tone/threshold — never drives warn or moderate, never enters `highNutrients`. Context-only line surfaces the raw %DV instead. `blood_sugar` condition's profileNote is unaffected (fires on the raw number — glycemic load, not added-sugar policy). Human-reviewed per item, requires `intrinsicSugarBasis`; never inferred from ingredient text | WHO 2015 free-sugars guideline explicitly excludes sugars intrinsic to whole fruit/vegetables — same citation as the added-sugar rule above, applied to the one case (restaurant data, no added-sugar field) the code didn't yet honor | A (regulatory consensus) |
