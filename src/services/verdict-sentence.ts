...
export function verdictSentence(input: SentenceInput): string | null {
  ...
  // the headline rather than letting a nutrient-dense-but-salty/fatty food read as a
  // consequence-free everyday pick. This is the "everyday, not all-day-everyday" case.
  if (budgetNutrient) {
    const name = budgetNutrient === 'sat fat' ? 'saturated fat' : 'sodium';
